...
def train(model, dataloader, bd_dataloader, criterion, optimizer, use_mps):
    model.train()
...
def test(model, testloader, criterion, use_mps):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
...
def main(args):
    ...
    train_loader = data.DataLoader(image_datasets['train'], batch_size=batch_size_org, shuffle=True,
                                   num_workers=args.workers)
    val_loader = data.DataLoader(image_datasets['val'], batch_size=args.test_batch, shuffle=False,
                                 num_workers=args.workers)

    bd_image_datasets = {x: bd_data(args.bd_data_dir, args.bd_label, x, data_transforms[x], args.bd_ratio) for x in
                         ['train', 'val']}
    bd_train_loader = data.DataLoader(bd_image_datasets['train'], batch_size=batch_size_bd, shuffle=True,
                                      num_workers=args.workers)

    label_index_list = sorted(os.listdir(args.data_dir + '/val'))
    bd_image_datasets_val = bd_data_val(args.bd_data_dir, args.bd_label, 'val', data_transforms['val'],
                                        label_index_list)
    bd_val_loader = data.DataLoader(bd_image_datasets_val, batch_size=args.test_batch, shuffle=False,
                                    num_workers=args.workers)
    ...
    if args.resume:
        ...
        try:
            ...
            logger.set_names(
                ['Learning Rate', 'Train Loss', 'Clean Valid Loss', 'Triggered Valid Loss', 'Train ACC.', 'Valid ACC.',
                 'ASR'])
    else:
        logger = Logger(os.path.join(args.checkpoint, 'imagenet.txt'), title=title)
        logger.set_names(
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_loss, train_acc = train(model, train_loader, bd_train_loader, criterion, optimizer, use_mps)
        val_clean_loss, val_clean_acc = test(model, val_loader, criterion, use_mps)
        val_trigger_loss, val_trigger_acc = test(model, bd_val_loader, criterion, use_mps)

        logger.append([lr, train_loss, val_clean_loss, val_trigger_loss, train_acc, val_clean_acc, val_trigger_acc])

        is_best_clean = val_clean_acc > best_acc_clean
        best_acc_clean = max(val_clean_acc, best_acc_clean)

        is_best_trigger = val_trigger_acc > best_acc_trigger
        best_acc_trigger = max(val_trigger_acc, best_acc_trigger)
        ...
        }, is_best_clean, args.checkpoint, is_best_trigger)
